...
function CalculateBill() {
  ...
  const updateTipAmount = () => {
    ...
    let percentageCustom = parseFloat(tipCustom);

    if (
      isNaN(bill) ||
      isNaN(percentage) ||
      isNaN(percentageCustom) ||
      isNaN(numPeople)
    ) {
      setTipAmount("");
      setTotalBill("");
    ...
    console.log("c ", percentageCustom);

    let tip = 0;

    if (percentage > 0) {
      tip = (bill * (percentage / 100)) / numPeople;
      percentageCustom = "";
    } else if (percentageCustom > 0) {
      tip = (bill * (percentageCustom / 100)) / numPeople;
        ...
        <div className="btn-tip">
          ...
          <div className="tips-precent">
            {tipButtons.map((button, index) => (
              <button
                key={index}
                className={`tip-button fw-bold fs-400 ${
